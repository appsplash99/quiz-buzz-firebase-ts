import React from "react";
// TODO: Check whether this is the correct type
import { RouteProps } from "react-router";
import { Route, Navigate } from "react-router-dom";
import { useSigninCheck } from "reactfire";
import { useToast } from "../context/toast-context";

export const PrivateRoute: React.FC<RouteProps> = ({ path, ...props }): React.ReactElement | null => {
  const { status, data: signInCheckResult } = useSigninCheck();
  const { toast, toggleToast } = useToast();

  if (status === "loading") {
    return (
      // TODO: ADD SPINNER LOADER HERE
      <div className="flex items-center justify-center bg-purple-600 mt-60 text-xl text-white font-mono font-bold px-2 py-1 rounded-full">
        <svg className="bg-red-300 animate-spin h-10 w-10 mr-3 ..." viewBox="0 0 24 24"></svg>
        <p>checking user login...</p>
      </div>
    );
  }

  if (signInCheckResult.signedIn === true) {
    // return <ProfilePage user={signInCheckResult.user} />;
    // TODO: might need to save user into context here

    return <Route {...props} path={path} />;
  } else {
    // return <SignInForm />;
    // toggleToast({ toast, toastType: "error", messege: "Please login" });
    return <Navigate state={{ from: path }} replace to="/login" />;
  }
};
